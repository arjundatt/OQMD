...
public class LeftHierarchy extends JPanel implements ActionListener, KeyListener {
    Map<HierarchyTreeNode, Connection> connectionMap = new HashMap<HierarchyTreeNode, Connection>();
    private JTree jTree;
    private JidePopupMenu rootPopUp;
    private JidePopupMenu instancePopUp;
    private JidePopupMenu schemaPopUp;
    private JidePopupMenu tablePopUp;
    ...
    private void createLeftHierarchy() {
        ...
        rootPopUp.add(temp);
        instancePopUp = new JidePopupMenu();
        ...
        temp.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (selectedNode.getChildCount() == 0) {
                    ArrayList<String> result = (ArrayList<String>) DatabaseUtility.getSchemas(connection);
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (Boolean.valueOf(selectedNode.getAttribute("metadata_discovered"))) {
                    ...
                    if (Boolean.valueOf(selectedNode.getAttribute("column_discovered"))) {
                        Vector<Vector<String>> result = DatabaseUtility.getMetaData(connection, selectedNode.getParent().toString(), selectedNode.toString());
                        Enumeration children = selectedNode.children();
                        int index = 0;
                        while (children.hasMoreElements()) {
                            HierarchyTreeNode colNode = (HierarchyTreeNode) children.nextElement();
                            Vector<String> colMetaData = result.elementAt(index);
                            colNode.setAttribute("type", colMetaData.elementAt(1));
                            colNode.setAttribute("length", colMetaData.elementAt(2));
                            index++;
                        }
    ...
    private void showConnectionDialog() {
        ...
        JideButton ok = addConnection.getOk();
    ...
    @Override
    public void actionPerformed(ActionEvent e) {
        if (addConnection.checkNotNullFields())
            getConnectionCall();
        else
    ...
    private void getConnectionCall() {
        try {
            connection = DatabaseUtility.getConnection(addConnection.getSystem().getText(), addConnection.getPort().getText(), addConnection.getInstanceName().getText(), addConnection.getUserName().getText(), addConnection.getPassword().getText());
        ...
        //TODO not working recursive connection addition
        /*int n = JOptionPane.showConfirmDialog(this.getTopLevelAncestor(), "Want to add more connections?", "Add More Connection", JOptionPane.YES_NO_CANCEL_OPTION);
        if (n == 0)
            addConnection.init();
        else {
            addConnection.close();
    ...
    private void callConnectionFromInstanceNode(HierarchyTreeNode instanceNode) {
        ...
        if (connection == null) {
            try {
                connection = DatabaseUtility.getConnection(instanceNode.getAttribute("system"), instanceNode.getAttribute("port"), instanceNode.getAttribute("instance"), instanceNode.getAttribute("uname"), instanceNode.getAttribute("pass"));
            } catch (SQLException e1) {
                e1.printStackTrace();
            }
        }
