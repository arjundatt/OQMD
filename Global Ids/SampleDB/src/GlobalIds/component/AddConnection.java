...
public class AddConnection extends JDialog implements ActionListener {
    ...
    private JideButton ok, cancel;
    private JideLabel messageDisplay;

    public JideLabel getMessageDisplay() {
        return messageDisplay;
    }

    public JideButton getOk() {
        return ok;
    }
    ...
    private void createAddConnection() {
        ...
        system = new TextField("System", "192.168.1.2", 34, true);
        fields.add(system);
        fields.add(Box.createVerticalStrut(20));
        instanceName = new TextField("Instance Name", 34, true);
        fields.add(instanceName);
        fields.add(Box.createVerticalStrut(20));
        comboBox = new JComboBox(new String[]{"Derby", "Oracle"});
        ...
        fields.add(new ComboBoxField("Database type", comboBox));
        fields.add(Box.createVerticalStrut(20));
        ...
        buttons = new JPanel();
        String format = "Fields marked ";
        String format2 = format("<html><font color='red'>%s</font></html>", "*");
        String format3 = " are compulsory";
        messageDisplay = new JideLabel(format + format2 + format3);
        ok = new JideButton("    Ok    ");
...
//        new AddConnection();
    ...
    public boolean checkNotNullFields() {
        if (connectionName.getText().equals(""))
            return false;
        if (system.getText().equals(""))
            return false;
        if (instanceName.getText().equals(""))
            return false;
    ...
    public void setErrorDisplay() {
        String format = format("<html><font color='red'>%s</font></html>", "The fields marked important cannot be null");
